...
def postprocess_bbbox(pred_bbox, ANCHORS, STRIDES, XYSCALE=[1,1,1]):
    ...
    return pred_bbox
def postprocess_boxes(pred_bbox, org_img_shape, input_size, score_threshold, classes_dict = read_class_names(cfg.YOLO.CLASSES)):
    ...
    for idx, cl in enumerate(classes):
        name = classes_dict[cl]
        if name in ['car', 'motorbike', 'truck', 'bus']:
            # print("===============================================")
            coors_out.append(list(coors[idx]))
            scores_out.append(scores[idx])
            classes_out.append(classes[idx])
    # print("********", coors_out, scores_out, "-----------\n", classes_out)
    if len(coors_out) == 0 or len(scores_out) == 0 or len(classes_out) == 0:
        return None
    return np.concatenate([np.array(coors_out), np.array(scores_out)[:, np.newaxis], np.array(classes_out)[:, np.newaxis]], axis=-1)
